...
assert dc["phase_profiles"]["init"] == "LM Studio Gemma E4B"
assert dc["phase_profiles"]["assess"] == "LM Studio Gemma E4B"
...
# 使用默认配置查找
p = server.find_profile(config, "LM Studio Gemma E4B")
assert p is not None
assert p["model"] == "google/gemma-4-e4b"
print(f"✅ find_profile('LM Studio Gemma E4B') → model={p['model']}")

p2 = server.find_profile_for_phase(config, "init")
assert p2 is not None
assert p2["model"] == "google/gemma-4-e4b"
